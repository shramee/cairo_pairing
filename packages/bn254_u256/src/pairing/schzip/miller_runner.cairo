...
use bn254_u256::{
    ...
    pairing::utils::{
        LnArrays, LnFn,
        {SZCommitment, SZCommitmentAccumulator, SZMillerRunner, SZAccumulator as Accumulator}
    }
};
...
> of MillerRunner<Curve, PreCompute<TSchZip>> {
    ...
    fn miller_bit_o(ref self: Curve, ref runner: PreCompute<TSchZip>, i: u32) { //
        ...
        self.sz_zero_bit(runner.schzip, ref runner.acc.schzip, ref runner.acc.f, (l1, l2, l3));
    }
    ...
    fn miller_bit_p(ref self: Curve, ref runner: PreCompute<TSchZip>, i: u32) {
        ...
        self
            .sz_nz_bit(
                runner.schzip,
                ref runner.acc.schzip,
                ref runner.acc.f,
                (l1, l2, l3),
                *g16.residue_witness_inv
            );
    }
    ...
    fn miller_last(ref self: Curve, ref runner: PreCompute<TSchZip>) { //
        ...
        let (l2, l3) = g16.lines.with_fxd_pt_lines(ref self, g16.ppc, ref runner.acc.line_index);
        self.sz_last_step(runner.schzip, ref runner.acc.schzip, ref runner.acc.f, (l1, l2, l3));
    }
}
